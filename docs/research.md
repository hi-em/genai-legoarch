...
Curated from a multi-agent research panel. Read the primary sources before implementing the matching module — several have exact algorithms/equations we should reuse rather than reinvent.

## Legolization (voxel → legal, buildable bricks) — core of `legolizer/bricks.py`
- **Luo et al., "Legolization: Optimizing LEGO Designs", SIGGRAPH Asia 2015.** Two-stage: voxelize, then split-and-remerge voxels into legal brick footprints (1×1…2×4…) while preserving silhouette; force-based stability metric + a refinement loop that re-merges the weakest region. → http://www.cmlab.csie.ntu.edu.tw/~forestking/research/SIGA15-Legolization/
...
## Color — `legolizer/color.py`
- Per-brick color must be assigned explicitly (stability/voxel tools are colorless). Map `legoarch` image colors → real LEGO palette via **CIEDE2000** nearest-neighbor. Without this, output is grey rubble.
